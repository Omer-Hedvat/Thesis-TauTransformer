...
def store_results(dataset, features_prc, features_to_reduce_prc, metric, acc, f1, classes, workdir):
    ...
    ds_results_mask = (
            ...
            (acc_results_df.features_to_reduce_prc == features_to_reduce_prc)
    )
    if ds_results_mask.any():
        ...
        new_df.loc[len(new_df), ['date', 'dataset', 'features_prc', 'features_to_reduce_prc', metric]] = \
            [today_date, dataset, features_prc, features_to_reduce_prc, (sum(acc) / len(acc))]
        acc_results_df = pd.concat([acc_results_df, new_df]).sort_values(by=['dataset', 'features_prc', 'features_to_reduce_prc'])
    acc_results_df.to_csv('results/all_datasets_results.csv', index=False)

    # Dataset's F1 Results File
    columns = ['features_prc', 'features_to_reduce_prc', *[f'{metric}_{class_name}' for class_name in classes]]
    values = [features_prc, features_to_reduce_prc, *f1]
    ...
    if not os.path.exists(f1_file):
        ...
        if all_ds_results_mask.any():
            ...
            f1_results_df = pd.concat([f1_results_df, new_data_df]).sort_values(by=['features_prc', 'features_to_reduce_prc'])
...
def all_results_colorful():
    data = pd.read_csv("results/all_datasets_results.csv")
    dat = data['dataset'] + " " + data['features_prc'].apply(str) + " " + data['features_to_reduce_prc'].apply(str)
    data['raw'] = dat
    data = data.set_index('raw')
    data = data.drop(columns=['date', 'dataset', 'features_prc', 'features_to_reduce_prc'])
...
def run_experiments(config):
    ...
    for feature_percentage, features_to_reduce_prc in \
            list(itertools.product(config['features_percentage'], config['features_to_reduce_prc'])):
        ...
        with features to reduce heuristic of {features_to_reduce_prc}%"""
                    )
        ...
        store_results(config['dataset_name'], feature_percentage, features_to_reduce_prc, 'all_features', all_features_acc, all_features_f1_agg, classes, workdir)
        ...
        store_results(config['dataset_name'], feature_percentage, features_to_reduce_prc, 'random_features', random_features_acc, random_features_f1_agg, classes, workdir)
        ...
        store_results(config['dataset_name'], feature_percentage, features_to_reduce_prc, 'fisher', fisher_features_acc, fisher_features_f1_agg, classes, workdir)
        ...
        store_results(config['dataset_name'], feature_percentage, features_to_reduce_prc, 'relief', relief_features_acc, relief_features_f1_agg, classes, workdir)
        ...
        store_results(config['dataset_name'], feature_percentage, features_to_reduce_prc, 'Chi_square', chi2_features_acc, chi2_features_f1_agg, classes, workdir)
        ...
        for dist in config['dist_functions']:
            ...
            coordinates, ranking = (diffusion_mapping(df_dists, config['alpha'], config['eps_type'], config['eps_factor']))
            ...
            store_results(config['dataset_name'], feature_percentage, features_to_reduce_prc, f'{dist}_kmeans', kmeans_acc, kmeans_f1_agg, classes, workdir)
